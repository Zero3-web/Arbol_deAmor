...
function showCountdown() {
  ...
  let baseEventDate = eventParam ? new Date(eventParam + 'T00:00:00') : new Date('2025-08-03T00:00:00');
  ...
  function update() {
    ...
    // Calcular el próximo aniversario (mismo mes y día, pero año actualizado)
    let nextAnniversary = new Date(baseEventDate);
    let currentYear = now.getFullYear();
    
    // Establecer el año del aniversario al año actual
    nextAnniversary.setFullYear(currentYear);
    
    // Si ya pasó la fecha de aniversario este año, avanzar al próximo año
    if (now >= nextAnniversary) {
      nextAnniversary.setFullYear(currentYear + 1);
    }
    
    let eventDiff = nextAnniversary - now;
