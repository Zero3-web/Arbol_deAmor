...
// Cargar el SVG y animar el árbol de amor
  ...
  .then(svgText => {
    ...
    setTimeout(() => {
      ...
      setTimeout(() => {
        ...
        // Cuando la animación de mover y escalar termine, solo cambiar fondo y letras (NO el SVG)
        svg.addEventListener('transitionend', function onMoveAndScale(e) {
          if (e.propertyName === 'transform') {
            ...
            // NO cambiar el SVG a blanco, mantener colores originales
            // Lanzar fuegos artificiales
            launchFireworks();
            // Mostrar mensaje, pétalos, cuenta regresiva y música SOLO después de mover el árbol
            setTimeout(() => {
              showDedicationText();
...
function showCountdown() {
  ...
  let eventDate = new Date(eventParam + 'T00:00:00');
  function update() {
    const now = new Date();
